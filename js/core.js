...
const UPDATE_NOTICES = [
  ...
  },
  {
    ...
    title:"대용량 양동이 저장 개선",
    lines:[
      "양동이를 여러 저장 공간으로 나누어 물고기가 많아도 안정적으로 저장합니다.",
      "보스전과 1대1 전투 기록도 계정 기록과 따로 저장해 보스전을 반복해도 저장 공간을 압박하지 않습니다.",
      "기존 계정은 로그인할 때 현재 물고기와 전투 기록을 새 저장 방식으로 자동 이전합니다."
    ]
  },
  {
    id:"2026-07-12-fishinglife-clear-text-25-0-2",
    title:"안내 문구 정리",
    lines:[
      "보스전과 여러 메뉴의 문장을 더 자연스럽고 알아보기 쉽게 다듬었습니다.",
      "게임 화면에 보이던 개발자용 표현을 쉬운 말로 바꿨습니다.",
      "인터넷 연결 문제는 저장 지연, 구버전 파일은 업데이트 필요로 구분해 표시합니다."
    ]
  },
  {
  ...
  },
  {
    ...
    title:"구버전 파일 차단·계정 기록 보호",
    lines:[
      "오래된 게임 파일로 접속하면 계정 기록이 바뀌지 않도록 저장을 막습니다.",
      "낚싯대 레벨·총 낚시 횟수·누적 수익이 이전 기록보다 낮아지면 저장하지 않습니다.",
      "문제가 생겼을 때 되돌릴 수 있도록 최근 계정 기록을 최대 3개까지 보관합니다.",
      "로그인 전에 최신 버전인지 자동으로 확인합니다. 업데이트가 필요하면 새로고침 안내가 표시됩니다."
    ]
  },
  {
    ...
    title:"계정 기록 복구 개선",
    lines:[
      "클라우드에 접속하기 전에 이 기기에 남은 물고기·지갑·레벨 기록을 안전하게 보관합니다.",
      "클라우드 기록과 이 기기 기록이 다르면 내용을 비교한 뒤 사용할 기록을 직접 선택할 수 있습니다.",
      "복구가 끝나기 전에는 기존 기록을 삭제하지 않습니다."
    ]
  },
  {
    ...
    lines:[
      "같은 아이디로 로그인하면 클라우드에 저장된 기록을 기준으로 게임을 불러옵니다.",
  ...
  },
  {
    ...
    lines:[
      ...
      "로그인 확인 과정을 줄여 게임 화면을 더 빠르게 엽니다.",
      "물고기 전투 기록은 조금씩 나누어 불러와 모바일 화면이 멈추지 않게 했습니다.",
      "로그인 후 받은 소식과 남은 게임 기록을 차례로 불러옵니다."
    ]
  },
  {
    id:"2026-07-11-fishinglife-cloud-save-boss-scene-24-4",
    title:"클라우드 저장·보스 공격 연출",
    lines:[
      "낚은 직후 저장 상태를 표시하고, 인터넷 연결이 끊기면 기록을 보관한 채 자동으로 다시 저장합니다.",
      "클라우드 기록과 이 기기 기록이 다를 때는 송금·물고기 선물과 최근 진행 내용을 안전하게 합칩니다.",
      "다른 기기에서 같은 계정으로 로그인해도 진행 중인 저장이 갑자기 사라지지 않게 보호합니다.",
      "보스가 공격하거나 스킬을 사용할 때 속성에 맞는 전투 배경이 잠시 펼쳐집니다."
    ]
  },
  {
    id:"2026-07-11-fishinglife-combat-readability-ranking-24-3",
    title:"전투 문구·게임 알림·랭킹 화면 개선",
    lines:[
      "1대1 전투 기록에 불필요한 내부 표시가 나타나던 문제를 수정했습니다.",
  ...
  },
  {
    ...
    lines:[
      ...
      "보스 난이도 선택과 1대1 출전 화면에서 저장한 파티를 바로 불러올 수 있습니다.",
  ...
  },
  {
    ...
    lines:[
      ...
      "모든 보스에 일반·어려움·크레이지 난이도가 추가되며, 앞 난이도를 깨면 다음 난이도가 열립니다.",
  ...
  },
  {
    ...
    title: "훈련소/낚싯대 확장",
    lines: [
      "훈련소 최대 레벨이 15단계로 늘어나며 별도의 조건 없이 골드로 강화할 수 있습니다.",
...
function clearOldAccountData(username){
  ...
  if(account){
    ...
    ].forEach(removeLargeLocalRecord);
    removeLargeLocalRecord("textFishingFullGame:"+account);
  }
...
function reportCloudVersionGate(message,status="update",title="업데이트 필요"){
  const detail=String(message||"게임이 업데이트되었습니다. 페이지를 새로고침해주세요.");
  ...
  setCloudSyncStatus(status,detail);
  if(!shouldReport)return;
  if(typeof globalThis.showFishingLifeNotice==="function")globalThis.showFishingLifeNotice({title,detail,kind:"danger",icon:"🛡️",duration:8000});
  else if(typeof print==="function")print(title+"\n\n"+detail);
  ...
  cloudVersionGatePromise=(async()=>{
    ...
    try{
      ...
      if(config.maintenance===true){
        reportCloudVersionGate(String(config.message||"현재 점검 중입니다. 잠시 후 다시 접속해주세요."),"maintenance","점검 중");
        return false;
      }
      if(minimumSchema>USER_WRITE_SCHEMA_VERSION||minimumProtocol>USER_WRITE_PROTOCOL_VERSION){
        reportCloudVersionGate(String(config.message||"게임이 업데이트되었습니다. 페이지를 새로고침해주세요."));
        return false;
      }
...
function isCloudConnectionError(error){
  const code=String(error?.code||"").toLowerCase();
  const message=String(error?.message||"").toLowerCase();
  return ["unavailable","firestore/unavailable","deadline-exceeded","firestore/deadline-exceeded","network-request-failed","auth/network-request-failed"].includes(code)
...
function handleProtectedWriteError(error,username=currentUser,localState=getGameState(),baseState=lastCloudSyncedState){
  ...
  reportCloudVersionGate("게임이 업데이트되었습니다. 현재 기록은 이 기기에 보관되어 있습니다. 페이지를 새로고침해주세요.");
  return true;
}
...
async function handleOnlineActionWriteError(error,username=currentUser,localState=getGameState(),baseState=lastCloudSyncedState){
  ...
  setCloudSyncStatus(hasPendingLocalCloudChanges()?"saving":"saved","내 계정 기록은 정상적으로 저장됩니다.");
  if(typeof globalThis.showFishingLifeNotice==="function")globalThis.showFishingLifeNotice({title:"상대 계정 확인 필요",detail:"상대가 이전 버전을 사용 중이어서 이번 작업을 완료하지 못했습니다.",kind:"danger",icon:"🛡️"});
  return true;
}
...
function accountBackupFingerprint(data){
  ...
  return JSON.stringify([
    ...
    Array.isArray(state.bucket)&&state.bucket.length?state.bucket.length:Number(state.bucketStorage?.count||state.bucketSummary?.count||0)
  ]);
}
...
function stageAccountBackup(tx,ref,existingData,reason,force=false){
  ...
  tx.set(backupRef,{
    ...
    summary:{
      ...
      bucketCount:Array.isArray(state.bucket)&&state.bucket.length?state.bucket.length:Number(state.bucketStorage?.count||state.bucketSummary?.count||0)
    },
    sourceTopLevel:{
      money:normalizeMoney(data.money??state.money??0),
      rodLevel:Math.max(1,Math.floor(Number(data.rodLevel??state.rodLevel??1))),
...
function bucketShardIdForFish(fish){
  const id=String(fish?.id||"");
  return "shard_"+String(stableStorageHash(id)%BUCKET_SHARD_COUNT).padStart(3,"0");
}

function ensureBucketStorageOrders(nextState,previousState={}){
  const nextBucket=Array.isArray(nextState?.bucket)?nextState.bucket:[],previousBucket=Array.isArray(previousState?.bucket)?previousState.bucket:[];
  const previousOrders=new Map();
  let maxOrder=0;
  previousBucket.forEach((fish,index)=>{
    if(!fish)return;
    const stored=Number(fish._bucketStorageOrder),order=Number.isFinite(stored)&&stored>0?stored:(index+1)*1000;
    previousOrders.set(String(fish.id||""),order);
    maxOrder=Math.max(maxOrder,order);
  });
  nextBucket.forEach(fish=>{
    const stored=Number(fish?._bucketStorageOrder);
    if(Number.isFinite(stored)&&stored>0)maxOrder=Math.max(maxOrder,stored);
  });
  const used=new Set();
  nextBucket.forEach(fish=>{
    if(!fish)return;
    const id=String(fish.id||""),inherited=previousOrders.get(id),stored=Number(fish._bucketStorageOrder);
    let order=Number.isFinite(inherited)&&inherited>0?inherited:(Number.isFinite(stored)&&stored>0?stored:0);
    if(!previousOrders.has(id)&&used.has(order))order=0;
    if(!order||used.has(order)){
      do{maxOrder+=1000;order=maxOrder;}while(used.has(order));
    }
    fish._bucketStorageOrder=order;
    used.add(order);
  });
  return nextState;
}

function partitionBucketForCloud(value){
  const result=new Map();
  (Array.isArray(value)?value:[]).forEach((fish,index)=>{
    if(!fish)return;
    const id=bucketShardIdForFish(fish),items=result.get(id)||[];
    const stored=Number(fish._bucketStorageOrder),order=Number.isFinite(stored)&&stored>0?stored:(index+1)*1000;
    items.push({order,fish:cloneCloudState(fish)});
    result.set(id,items);
  });
  return result;
}

function cloudFishPower(fish){
  const combat=fish?.combat&&typeof fish.combat==="object"?fish.combat:{};
  const attack=Math.max(0,Number(combat.attack||0)),maxHp=Math.max(0,Number(combat.maxHp??combat.hp??0));
  const critRate=Math.max(0,Number(combat.critRate||0)),critDamage=Math.max(100,Number(combat.critDamage||150));
  return attack*(1+critRate/100*Math.max(0,critDamage/100-1))+maxHp*.2;
}

function makeCloudBucketSummary(state){
  const source=state&&typeof state==="object"?state:{},list=(Array.isArray(source.bucket)?source.bucket:[]).filter(Boolean);
  const aquariumIds=normalizeAquariumFishIds(source.aquariumFishIds),pvpIds=normalizePartyPresets(source.partyPresets).pvp;
  const byId=new Map(list.map(fish=>[String(fish.id||""),fish]));
  const aquarium=aquariumIds.map(id=>byId.get(id)).filter(Boolean).slice(0,5).map(cloneCloudState);
  let pvp=pvpIds.map(id=>byId.get(id)).filter(fish=>fish&&fish.grade!=="쓰레기").slice(0,3);
  if(pvp.length<3)pvp=[...list].filter(fish=>fish.grade!=="쓰레기"&&fish.combat).sort((a,b)=>cloudFishPower(b)-cloudFishPower(a)).slice(0,3);
...
function prepareCloudGameState(fullState,existingCloudState={},includeSplitData=false){
  const full=cloneCloudState(fullState&&typeof fullState==="object"?fullState:{})||{},existing=existingCloudState&&typeof existingCloudState==="object"?existingCloudState:{};
  const history=normalizeBattleHistory(full.battleHistory);
  full.bucket=[];
  full.battleHistory={boss:[],pvp:[]};
  if(includeSplitData){
    const summary=makeCloudBucketSummary(fullState);
    full.bucketStorage={version:BUCKET_STORAGE_VERSION,shardCount:BUCKET_SHARD_COUNT,count:summary.count};
    full.bucketSummary=summary;
    full.battleStorage={version:BATTLE_STORAGE_VERSION,bossCount:history.boss.length,pvpCount:history.pvp.length};
  }else{
    full.bucketStorage=cloneCloudState(existing.bucketStorage||full.bucketStorage||{version:BUCKET_STORAGE_VERSION,shardCount:BUCKET_SHARD_COUNT,count:0});
    full.bucketSummary=cloneCloudState(existing.bucketSummary||full.bucketSummary||{count:0,aquarium:[],pvp:[]});
    full.battleStorage=cloneCloudState(existing.battleStorage||full.battleStorage||{version:BATTLE_STORAGE_VERSION,bossCount:0,pvpCount:0});
  }
  full.saveSchemaVersion=USER_WRITE_SCHEMA_VERSION;
  return full;
}
...
async function loadSplitGameState(username,userData){
  const data=userData&&typeof userData==="object"?userData:{},state=cloneCloudState(data.gameState||createFreshAccountState())||createFreshAccountState();
  const userRef=db.collection("users").doc(String(username||data.nickname||""));
  if(Number(state.bucketStorage?.version||0)>=BUCKET_STORAGE_VERSION){
    const snapshot=await userRef.collection(BUCKET_SHARD_COLLECTION).get(),ordered=[];
    snapshot.docs.forEach(doc=>{
      const shard=doc.data()||{},items=Array.isArray(shard.items)?shard.items:[];
      items.forEach(item=>{
        if(!item?.fish)return;
        const fish=cloneCloudState(item.fish),order=Number(item.order||0);
        if(order>0)fish._bucketStorageOrder=order;
        ordered.push({order,fish});
      });
    });
...
globalThis.loadSplitGameState=loadSplitGameState;
...
function txSetProtectedUser(tx,ref,existingData,patch,reason="user_write",options={}){
  ...
  const fullState=protectedUserState(existingData,patch||{});
  const existingCloudState=existingData?.gameState&&typeof existingData.gameState==="object"?existingData.gameState:{};
  const legacyHistory=normalizeBattleHistory(fullState.battleHistory);
  const containsLegacySplitData=(Array.isArray(fullState.bucket)&&fullState.bucket.length>0)||((legacyHistory.boss.length+legacyHistory.pvp.length)>0&&Number(existingCloudState.battleStorage?.version||0)<BATTLE_STORAGE_VERSION);
  const includeSplitData=options.fullStateIncludesSplitData===true||create||options.allowAccountReset===true||containsLegacySplitData;
  const previousState=options.previousState&&typeof options.previousState==="object"?options.previousState:(containsLegacySplitData?existingCloudState:{});
  if(includeSplitData)ensureBucketStorageOrders(fullState,previousState);
  const state=prepareCloudGameState(fullState,existingCloudState,includeSplitData);
  assertCloudStateSize(state);
  if(options.allowAccountReset!==true)assertNoProgressRegression(existingData,fullState);
  ...
  Object.defineProperty(result,"fullGameState",{value:cloneCloudState(fullState),enumerable:false});
  return result;
}
...
function setCloudSyncStatus(status,detail=""){
  ...
  const config={offline:["☁️","로그인 필요"],saving:["↻","저장 중"],saved:["✓","저장 완료"],retry:["!","저장 지연"],retrying:["↻","다시 저장 중"],error:["!","확인 필요"],update:["!","업데이트 필요"],connection:["!","연결 확인"],maintenance:["!","점검 중"]}[status]||["☁️","클라우드"];
...
function scheduleCloudSaveRetry(connectionIssue=false){
  ...
  setCloudSyncStatus(connectionIssue?"retry":"retrying",`${Math.ceil(delay/1000)}초 뒤 다시 저장합니다.`);
...
const LARGE_LOCAL_DB_NAME="FishingLifeLocalData";
const LARGE_LOCAL_DB_STORE="records";
const MAX_LOCAL_STORAGE_RECORD_BYTES=900000;
let largeLocalDbPromise=null;

function openLargeLocalDatabase(){
  if(largeLocalDbPromise)return largeLocalDbPromise;
  largeLocalDbPromise=new Promise(resolve=>{
    if(typeof indexedDB==="undefined")return resolve(null);
    const request=indexedDB.open(LARGE_LOCAL_DB_NAME,1);
    request.onupgradeneeded=()=>{
      const database=request.result;
      if(!database.objectStoreNames.contains(LARGE_LOCAL_DB_STORE))database.createObjectStore(LARGE_LOCAL_DB_STORE,{keyPath:"key"});
    };
    request.onsuccess=()=>resolve(request.result);
    request.onerror=()=>{console.error(request.error);resolve(null);};
  });
  return largeLocalDbPromise;
}

function largeLocalRecordKey(key){return "v1:"+String(key||"");}

async function writeLargeLocalRecord(key,value){
  const database=await openLargeLocalDatabase();
  if(!database)return false;
  return new Promise(resolve=>{
    try{
      const transaction=database.transaction(LARGE_LOCAL_DB_STORE,"readwrite");
      transaction.objectStore(LARGE_LOCAL_DB_STORE).put({key:largeLocalRecordKey(key),value:cloneCloudState(value),savedAt:Date.now()});
      transaction.oncomplete=()=>resolve(true);
      transaction.onerror=()=>{console.error(transaction.error);resolve(false);};
      transaction.onabort=()=>resolve(false);
    }catch(error){console.error(error);resolve(false);}
  });
}

async function readLargeLocalRecord(key){
  const database=await openLargeLocalDatabase();
  if(!database)return null;
  return new Promise(resolve=>{
    try{
      const request=database.transaction(LARGE_LOCAL_DB_STORE,"readonly").objectStore(LARGE_LOCAL_DB_STORE).get(largeLocalRecordKey(key));
...
function removeLargeLocalRecord(key){
  localStorage.removeItem(key);
  openLargeLocalDatabase().then(database=>{
    if(!database)return;
    try{database.transaction(LARGE_LOCAL_DB_STORE,"readwrite").objectStore(LARGE_LOCAL_DB_STORE).delete(largeLocalRecordKey(key));}
    catch(error){console.error(error);}
  });
}

function persistLargeLocalRecord(key,value){
  const snapshot=cloneCloudState(value);
  writeLargeLocalRecord(key,snapshot).catch(console.error);
  try{
    const serialized=JSON.stringify(snapshot);
    if(new TextEncoder().encode(serialized).byteLength<=MAX_LOCAL_STORAGE_RECORD_BYTES)localStorage.setItem(key,serialized);
    else localStorage.removeItem(key);
    return true;
  }catch(error){console.error(error);try{localStorage.removeItem(key);}catch(ignore){}return false;}
}

function compactLocalGameSnapshot(state){
  const compact=cloneCloudState(state&&typeof state==="object"?state:{})||{};
  const history=normalizeBattleHistory(compact.battleHistory);
  compact.localSnapshotSummary={
    bucketCount:Array.isArray(compact.bucket)?compact.bucket.length:0,
    bossBattleCount:history.boss.length,
    pvpBattleCount:history.pvp.length,
    fullStateInLargeStorage:true
  };
  compact.bucket=[];
  compact.battleHistory={boss:[],pvp:[]};
  return compact;
}

function persistLocalGameSnapshot(state=getGameState()){
  if(!currentUser)return false;
  const snapshot=cloneCloudState(state);
  writeLargeLocalRecord("textFishingFullGame:"+currentUser,snapshot).catch(console.error);
  try{
    const fullSerialized=JSON.stringify(snapshot);
    const localValue=new TextEncoder().encode(fullSerialized).byteLength<=MAX_LOCAL_STORAGE_RECORD_BYTES?snapshot:compactLocalGameSnapshot(snapshot);
    localStorage.setItem("textFishingSpeciesSizeSave",JSON.stringify(localValue));
    return true;
...
function persistCloudSyncBase(username,state=lastCloudSyncedState,revision=cloudRevision){
  ...
  return persistLargeLocalRecord(cloudBaseKey(username),{state:cloneCloudState(state),revision:Number(revision||0),savedAt:Date.now(),schemaVersion:USER_WRITE_SCHEMA_VERSION});
}
...
function storeCloudRecovery(username,localState=getGameState(),baseState=lastCloudSyncedState,dirty=true){
  ...
  try{
    ...
    return persistLargeLocalRecord("textFishingCloudRecovery:"+username,{
      username,
...
function discardCloudRecovery(username){if(username)removeLargeLocalRecord("textFishingCloudRecovery:"+username);}
...
async function readCloudRecovery(username){
  const local=peekCloudRecovery(username);
  if(local)return local;
  username=cleanNickname(username);
  if(!username)return null;
  const value=await readLargeLocalRecord("textFishingCloudRecovery:"+username);
  const valid=value?.dirty===true
    && cleanNickname(value?.username)===username
    && Number(value?.accountResetVersion||0)===ACCOUNT_RESET_VERSION
    && Number(value?.schemaVersion||0)===USER_WRITE_SCHEMA_VERSION
...
function captureStartupEmergencySnapshot(username,state,source="이 브라우저에 남은 시작 데이터",baseState=null){
  ...
  const nextSnapshot={
    ...
    signature,
    signatureVersion:2,
    source:String(source||"이 기기에 남은 기록"),
    capturedAt:Date.now(),
...
function keepCurrentFirebaseEmergencyState(candidateId=""){
  ...
  if(!lastCloudSyncedState)return {ok:false,message:"클라우드 기록을 아직 불러오지 못했습니다."};
  ...
  persistLocalGameSnapshot(getGameState());
  clearPersistentCloudDirty(currentUser);
  discardCloudRecovery(currentUser);
  dismissStartupEmergencySnapshot();
  setCloudSyncStatus("saved","클라우드 기록으로 계속합니다.");
  updateWallet();
...
async function restoreStartupEmergencySnapshot(candidateId=""){
  ...
  if(!(await checkGameVersion()))return {ok:false,error:"UPDATE_REQUIRED",message:"페이지를 새로고침한 뒤 다시 시도해주세요."};
  const snapshot=getStartupEmergencySnapshot();
  ...
  try{
    ...
    const initialServerSnap=await ref.get();
    if(!initialServerSnap.exists)throw new Error("ACCOUNT_NOT_FOUND");
    const initialServerData=initialServerSnap.data()||{},initialServerRevision=Number(initialServerData.cloudRevision||0);
    const fullServerState=await loadSplitGameState(account,initialServerData);
    const commitEmergencyRestore=()=>db.runTransaction(async tx=>{
      ...
      if(Number(data.cloudRevision||0)!==initialServerRevision)throw new Error("SERVER_CHANGED");
      const serverState=fullServerState;
      ...
      },"emergency_restore",{forceBackup:true,fullStateIncludesSplitData:true,previousState:serverState});
    ...
    dismissStartupEmergencySnapshot();
    updateWallet();
    setCloudSyncStatus("saved","선택한 이 기기 기록으로 복구했습니다.");
    if(typeof globalThis.requestFishingLifeRender==="function")globalThis.requestFishingLifeRender(true);
    ...
    if(currentUser===account){
      if(e.message==="SERVER_CHANGED"&&latestServerState){
        ...
        if(!shouldProtectEmergencyOriginalLocalSave())persistLocalGameSnapshot(getGameState());
    ...
    updateWallet();
    setCloudSyncStatus("error","복구를 완료하지 못했습니다. 기존 기록은 그대로 보관되어 있습니다.");
    const message=e.message==="SESSION_INVALID"?"로그인 정보가 만료되었습니다. 다시 로그인해주세요.":e.message==="ACCOUNT_NOT_FOUND"?"계정을 찾지 못했습니다.":e.message==="ACCOUNT_CHANGED"?"로그인 계정이 바뀌어 복구를 중단했습니다.":e.message==="SERVER_CHANGED"?"클라우드 기록이 새로 바뀌었습니다. 내용을 다시 확인해주세요.":"인터넷 연결 문제로 복구를 완료하지 못했습니다. 기존 기록은 그대로 보관되어 있습니다.";
...
function saveCloudData(){
  ...
  if(hasPendingEmergencyRecoveryDecision()){
    setCloudSyncStatus("error","사용할 게임 기록을 선택할 때까지 저장을 잠시 멈춥니다.");
...
async function saveCloudDataNow(){
  ...
  const sessionHash = await getCurrentSessionHash();
  if(!sessionHash){
    setCloudSyncStatus("error","로그인 정보를 확인할 수 없습니다. 다시 로그인해주세요.");
    return false;
  }
  ...
  try{
    ...
    if(currentUser === userAtStart){
      ...
      setCloudSyncStatus(pending?"saving":"saved",pending?"새 변경 내용을 계속 저장하고 있습니다.":"클라우드 저장이 완료되었습니다.");
    }
  }catch(e){
    console.error(e);
    if(e.message === "SESSION_INVALID"){
      ...
      if(typeof globalThis.showFishingLifeNotice==="function")globalThis.showFishingLifeNotice({title:"계정 기록 보호",detail:"더 최근에 저장된 클라우드 기록을 보호했습니다. 이 기기 기록은 복구용으로 보관되어 있습니다.",kind:"danger",icon:"🛡️"});
    }else if(e.message === "USER_STATE_TOO_LARGE"){
      storeCloudRecovery(userAtStart,localState,baseState,true);
      setCloudSyncStatus("error","게임 기록이 너무 많아 클라우드에 저장하지 못했습니다.");
      if(typeof globalThis.showFishingLifeNotice==="function")globalThis.showFishingLifeNotice({title:"저장 공간 확인 필요",detail:"물고기와 전투 기록이 많아 저장하지 못했습니다. 현재 기록은 이 기기에 보관되어 있습니다.",kind:"danger",icon:"🗄️",duration:8000});
    }else if(handleProtectedWriteError(e,userAtStart,localState,baseState)){
      return false;
    }else{
      const connectionIssue=isCloudConnectionError(e);
      scheduleCloudSaveRetry(connectionIssue);
      if(cloudSaveRetryAttempt===1&&typeof globalThis.showFishingLifeNotice==="function")globalThis.showFishingLifeNotice(connectionIssue
        ?{title:"클라우드 저장 지연",detail:"인터넷 연결을 확인해주세요. 기록은 이 기기에 보관되며 자동으로 다시 저장합니다.",kind:"danger",icon:"☁️"}
        :{title:"저장을 다시 확인합니다",detail:"현재 기록은 이 기기에 보관되어 있으며 잠시 후 다시 저장합니다.",kind:"info",icon:"↻"});
    }
  }
}
...
async function refreshMyCloudData(force=false){
  ...
  try{
    ...
    const serverState=await loadSplitGameState(currentUser,data);
    const nextState=hadPending?mergeCloudGameStates(getGameState(),serverState,lastCloudSyncedState):serverState;
    applyGameState(nextState);
    cloudRevision = Number(data.cloudRevision || 0);
    lastCloudSyncedState=cloneCloudState(serverState);
    persistCloudSyncBase(currentUser,serverState,cloudRevision);
    if(!hadPending)cloudSyncedSeq = localSaveSeq;
    migrateCombatStatsToCurrentVersion();
    persistLocalGameSnapshot(getGameState());
    updateWallet();
    if(hadPending)saveCloudData();
    else{clearPersistentCloudDirty(currentUser);setCloudSyncStatus("saved","클라우드의 최신 기록을 불러왔습니다.");}
...
async function removeRealtimeNotification(matchText){
  ...
  try{
    ...
    await db.runTransaction(async (tx)=>{
      ...
      const data = snap.data();
      if(!isSessionHashValid(data,sessionHash)) throw new Error("SESSION_INVALID");
      const cloudState=data.gameState&&typeof data.gameState==="object"?data.gameState:{};
      const list = Array.isArray(cloudState.notifications) ? cloudState.notifications : [];
      const filtered = list.filter(msg => !String(msg).includes(matchText));
      committedState = {...localState, notifications: filtered};
      const write=txSetProtectedUser(tx,ref,data,{
        cloudRevision: Number(data.cloudRevision || 0) + 1,
        gameState: {...cloudState,notifications:filtered}
      },"notification_read",{previousState:localState});
      committedRevision = write.cloudRevision;
    });
    ...
    if(e.message==="USER_STATE_TOO_LARGE"){
      ...
      setCloudSyncStatus("error","게임 기록이 너무 많아 새 소식을 저장하지 못했습니다.");
    }else if(!handleProtectedWriteError(e,currentUser,localState,baseState))scheduleCloudSaveRetry(isCloudConnectionError(e));
  }
}
...
async function registerUser(providedNickname,providedPassword){
  ...
  try{
    ...
    persistCloudSyncBase(currentUser,lastCloudSyncedState,cloudRevision);
    clearPersistentCloudDirty(currentUser);
    setCloudSyncStatus("saved","새 계정을 클라우드에 저장했습니다.");
...
async function loginUser(providedNickname,providedPassword){
  ...
  try{
    ...
    await db.runTransaction(async tx=>{
      ...
      const serverState=accountWasReset?createFreshAccountState():cloneCloudState(initialState||createFreshAccountState());
...
async function logoutUser(){
  ...
  if(hasPendingLocalCloudChanges()){
    ...
    return print("아직 저장 중입니다. 기록을 보호하기 위해 로그아웃을 잠시 멈췄습니다.");
  }
  stopServerAlertListener();
  stopOnlinePresence();
...
function closeModal(force=false){
  ...
  if(!force&&emergencyRestoreRunning){
    if(typeof globalThis.showFishingLifeNotice==="function")globalThis.showFishingLifeNotice({title:"계정 기록 복구 중",detail:"기록 저장이 끝날 때까지 잠시 기다려주세요.",kind:"info",icon:"🛟"});
    return;
  }
  if(!force&&typeof hasPendingEmergencyRecoveryDecision==="function"&&hasPendingEmergencyRecoveryDecision()){
    if(typeof globalThis.showFishingLifeNotice==="function")globalThis.showFishingLifeNotice({title:"사용할 기록 선택",detail:"클라우드 기록과 이 기기 기록 중 하나를 선택해주세요.",kind:"info",icon:"🛟"});
