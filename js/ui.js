(() => {
  ...
  function openVersionBlocker(detail=""){
    const message=String(detail||cloudVersionGateMessage||"Firebase 서버에서 안전 버전을 확인해야 합니다.");
    openUiModal("FishingLife 안전 확인",`<div class="game-dialog"><div class="dialog-summary warning"><div><small>ACCOUNT PROTECTION</small><b>게임 저장을 잠시 잠갔습니다.</b><p>${safe(message)}</p></div><span>🛡️</span></div><div class="confirm-danger"><b>이 상태에서는 낚시·판매·강화·전송을 진행할 수 없습니다.</b><p>네트워크를 확인한 뒤 안전 버전을 다시 확인해주세요. 이 기기에 남아 있던 복구 기록은 유지됩니다.</p></div><div id="versionGateRetryStatus" class="login-gate-status" role="status">Firebase 서버 확인이 필요합니다.</div><div class="dialog-actions"><button class="primary" data-version-gate-retry>안전 버전 다시 확인</button></div></div>`);
  }
  ...
  function emergencyRecoveryStateCard(kind,label,summary,featured=true){
    return `<article class="emergency-recovery-card ${kind}"><header><small>${kind==="local"?"DEVICE RECOVERY":"FIREBASE CURRENT"}</small><b>${safe(label)}</b></header><div class="emergency-recovery-stats"><span><small>낚싯대</small><strong>Lv.${Number(summary.rodLevel||1).toLocaleString()}</strong></span><span><small>지갑</small><strong>${safe(formatMoney(summary.money||0))}</strong></span><span><small>물고기</small><strong>${Number(summary.fishCount||0).toLocaleString()}마리</strong></span><span><small>총 낚시</small><strong>${Number(summary.totalFishingCount||0).toLocaleString()}회</strong></span><span class="wide"><small>PVP 총전투력 · 상위/프리셋 3마리</small><strong>${Number(summary.pvpPower||0)>0?safe(compactNumber(summary.pvpPower)):"저장 수치 없음"}</strong></span></div>${emergencyRecoveryGradeHtml(summary)}${featured?`<div class="emergency-recovery-featured"><small>등급이 높은 물고기 최대 5마리</small>${emergencyRecoveryFeaturedHtml(summary)}</div>`:""}</article>`;
  }
  ...
  function replayActionHtml(value){
    ...
    return html
      .replace(/치명타!/g,'<span class="battle-result-badge critical">💥 CRITICAL</span>')
      .replace(/회피(?:했습니다\.|!|(?=<br>|$))/g,'<span class="battle-result-badge dodge">💨 DODGE</span>')
  ...
  function replayImpactBadge(value){
    ...
    if(text.includes("치명타"))return '<span class="battle-result-badge critical">💥 CRITICAL</span>';
    if(/회피(?:!|했습니다|$)/.test(text))return '<span class="battle-result-badge dodge">💨 DODGE</span>';
    return "";
  }
  ...
  const voidReplayVariants=[
    {key:"letter-one",className:"battle-event--void-letter-one",label:"VOID CHAPTER I"},
    {key:"letter-two",className:"battle-event--void-letter-two",label:"VOID CHAPTER II"},
    {key:"letter-three",className:"battle-event--void-letter-three",label:"VOID FINAL CHAPTER"},
    {key:"observer",className:"battle-event--void-observer",label:"VOID OBSERVATION"},
    {key:"anomaly",className:"battle-event--void-anomaly",label:"SYSTEM ERROR"}
  ];
  ...
  function prepareBossReplayFrames(rawFrames){
    ...
    for(let i=0;i<source.length;i++){
      const frame=source[i],kind=replayEventKind(frame?.entry),voidVariant=getVoidReplayVariant(frame?.entry),isVoid=!!voidVariant||String(frame?.entry||"").includes("battle-event--void");
      if(!["skill","crazy"].includes(kind)&&!isVoid){skillFrames.push(frame);continue;}
      ...
      const impactKind=nextText.includes("치명타")?"critical":/회피(?:!|했습니다|$)/.test(nextText)?"dodge":"",merged=resultRows.length?{...frame,summons:next.summons||frame.summons,skillDetail,impactKind}:{...next,entry:frame.entry,skillDetail,impactKind};
      skillFrames.push(merged);
      if(resultRows.length)skillFrames.push({...next,skillResult:true,skillResultRows:resultRows,skillSourceKind:isVoid?"void":kind,skillSourceVariant:voidVariant?.key||""});
      i++;
    }
  ...
  function renderReplaySummons(summons,entry=""){
    ...
    host.innerHTML=list.length?`<small class="replay-summon-title">BOSS SUMMONS · ${list.length}</small><div>${list.map(s=>{
      ...
      return `<article class="replay-summon ${targeted?"targeted":""} ${spawned?"spawned":""}"><span>${replaySummonIcon(s.name)}</span><div><small>${spawned?"SUMMONED":"BOSS MINION"}</small><b>${safe(label)}</b><em>⚔ ${compactNumber(s.attack)}</em><div class="replay-hp summon-hp"><i style="width:${hpPercent(s.hp,s.maxHp)}%"></i></div><em>${combatHpNumber(s.hp)} / ${combatHpNumber(s.maxHp)}</em></div></article>`;
    }).join("")}</div>`:"";
  ...
  function updateReplayFrame(frame){
    ...
    const bossBar=dom.bossBar||$("#replayBossHpBar"),bossHp=dom.bossHp||$("#replayBossHpText");
    ...
    (frame.fish||[]).forEach((fish,index)=>{
      const card=dom.fishCards?.[index]||$(`[data-replay-fish-index="${index}"]`),bar=dom.fishBars?.[index]||$(`[data-replay-fish-bar="${index}"]`),hp=dom.fishHp?.[index]||$(`[data-replay-fish-hp="${index}"]`);
    ...
    if(isSkillResult){
      ...
      const damageResult=frame.skillSourceKind==="damage",actionHtml=`<section class="battle-skill-result"><small>${damageResult?"DAMAGE RESULT":"SKILL HP RESULT"}</small><h3>${damageResult?"피해 적용 결과":"스킬 적용 결과"}</h3>${rows||`<p>${safe(replayDetailText(frame.entry))}</p>`}</section>`;
      if(actionHtml!==state.lastActionHtml){action.innerHTML=actionHtml;state.lastActionHtml=actionHtml;}
      return;
    }

    action.className="battle-action-card "+(kind!=="normal"?"is-"+kind:"")+(isVoid?" is-void":"")+(voidVariant?" is-void-"+voidVariant.key:"");
    const bossEventLabel=kind==="crazy"?"CRAZY ULTIMATE":kind==="phase"?"BOSS PHASE":kind==="passive"?"BOSS PASSIVE":"BOSS SKILL";
    const actorBanner=attacker?`<div class="battle-attacker-banner ${kind==="ally"?"ally":""} ${isVoid?"void":""}"><span>${fishIcon(attacker)}</span><div><small>${isVoid?(voidVariant?.label||"VOID SIGNATURE"):kind==="ally"?"ALLY SKILL":"PLAYER ATTACK"}</small><b>${safe(attacker.battleLabel||attacker.name)}</b></div></div>`:["skill","crazy","passive","phase"].includes(kind)?`<div class="battle-attacker-banner boss"><span>${safe(arena.dataset.bossSymbol||"🐲")}</span><div><small>${bossEventLabel}</small><b>${safe(bossName)}</b></div></div>`:bossAttack?`<div class="battle-attacker-banner boss"><span>${safe(arena.dataset.bossSymbol||"🐲")}</span><div><small>BOSS ATTACK</small><b>${safe(bossName)}</b></div></div>`:"";
    const detail=frame.skillDetail?`<div class="battle-skill-detail"><small>SKILL EFFECT</small>${replayImpactBadge(frame.skillDetail)}<p>${safe(frame.skillDetail).replace(/\n/g,"<br>")}</p></div>`:"";
  ...
  function updatePvpReplayFrame(frame){
    ...
    const mine=frame[dom.mineSide]||[],enemy=frame[dom.enemySide]||[],displayEntry=pvpEntryForViewer(frame.entry,dom.mineSide,dom.legacySwap),voidVariant=getVoidReplayVariant(frame.entry),isVoid=String(frame.entry||"").includes("battle-event--void"),isSkillResult=!!frame.skillResult,text=plain(`${displayEntry} ${frame.skillDetail||""}`),actorIsMine=frame.actorSide===dom.mineSide;
    dom.arena.classList.remove("hit-ally","hit-enemy","event-skill","event-void","event-crit","event-dodge","event-result");clearVoidReplayClasses(dom.arena);clearGradeAttackEffect(dom.arena);
    ...
    if(isSkillResult){
      ...
      const detail=replayDetailText(displayEntry),damageResult=frame.skillSourceKind==="damage",actionHtml=`<section class="battle-skill-result"><small>${damageResult?"DAMAGE RESULT":"ACTION HP RESULT"}</small><h3>${damageResult?"피해 적용 결과":"스킬 적용 결과"}</h3>${rows||`<p>${safe(detail||"체력 변화 없이 효과가 적용되었습니다.")}</p>`}</section>`;
    ...
    const actorHtml=actor?`<div class="battle-attacker-banner ${actorIsMine?"ally":"boss"} ${isVoid?"void":""}"><span>${fishIcon(actor)}</span><div><small>${isVoid?(voidVariant?.label||"VOID SIGNATURE"):actorIsMine?"MY ATTACK":"RIVAL ATTACK"}</small><b>${safe(actor.name)}</b></div></div>`:"";
    const detail=frame.skillDetail?`<div class="battle-skill-detail"><small>SKILL EFFECT</small>${replayImpactBadge(frame.skillDetail)}<p>${safe(frame.skillDetail).replace(/\n/g,"<br>")}</p></div>`:frame.impactKind?`<div class="battle-skill-detail impact-only">${frame.impactKind==="critical"?'<span class="battle-result-badge critical">💥 CRITICAL</span>':'<span class="battle-result-badge dodge">💨 DODGE</span>'}</div>`:"";
  ...
  function finishPvpBattleReplay(replay){
    ...
    dom.action.innerHTML=`<div class="battle-finish-card ${draw?"draw":won?"win":"lose"}"><small>PVP BATTLE END</small><strong>${draw?"무승부":won?"승리":"패배"}</strong><p>${legacyTextHtml(replay.summary||"")}</p><div class="battle-finish-actions"><button data-pvp-replay>다시 보기</button><button data-pvp-replay-close>닫기</button></div></div>`;
    dom.turnLabel.textContent="BATTLE END";
  ...
  async function playPvpBattleReplay(replay){
    ...
    const intro=$("#pvpBattleIntro"),countdown=$("#pvpIntroCountdown");
    if(intro){
      for(let remaining=7;remaining>=1;remaining--){
        if(token!==state.pvpReplayToken||$("#modalOverlay")?.style.display==="none")return;
        if(countdown)countdown.textContent=String(remaining);
        if(state.pvpReplayDom?.turnLabel)state.pvpReplayDom.turnLabel.textContent=`MATCH PREVIEW · ${remaining}`;
        await sleep(1000);
        if(state.pvpReplaySkip)break;
      }
      intro.hidden=true;state.pvpReplayDom?.arena?.classList.remove("is-intro");
      if(state.pvpReplayDom?.turnLabel)state.pvpReplayDom.turnLabel.textContent="BATTLE START";
    }
  ...
  function openPvpBattleReplay(replay){
    ...
    openUiModal("1대1 라이브 전투",`<div class="pvp-battle-replay"><header class="battle-replay-head"><div><small>LIVE PVP BATTLE</small><b id="pvpBattleTurnLabel">MATCH PREVIEW · 7</b></div><div class="battle-speed"><button class="active" data-pvp-battle-speed="1">×1</button><button data-pvp-battle-speed="2">×2</button><button data-pvp-battle-speed="4">×4</button><button data-pvp-battle-skip>건너뛰기</button></div></header><section id="pvpBattleArena" class="pvp-battle-arena is-intro"><div class="grade-attack-layer" aria-hidden="true"></div><div id="pvpBattleIntro" class="pvp-battle-intro"><header><small>PVP MATCH ANALYSIS</small><h2>출전 파티 비교</h2><p><strong id="pvpIntroCountdown">7</strong>초 후 교대 전투가 시작됩니다.</p></header><div class="pvp-intro-matchup">${pvpIntroTeamHtml(mineTeam,mine.name,"MY TEAM")}<strong>VS</strong>${pvpIntroTeamHtml(enemyTeam,enemy.name,"RIVAL TEAM")}</div><i class="pvp-intro-timer"></i></div><section class="pvp-battle-side ally"><div class="battle-player-card"><div class="${mineProfile.classes}" style="${mineProfile.style}" data-aura-symbol="${safe(mineProfile.aura)}"><span>${mineProfile.avatar}</span></div><div><small>MY CAPTAIN</small><b>${safe(mine.name||"나")}</b><em>${safe(mine.title?`[${mine.title}]`:"칭호 없음")}</em></div></div><div class="pvp-live-team">${pvpReplayTeamHtml(mineTeam,mineSide)}</div></section><div class="battle-versus">VS</div><section class="pvp-battle-side enemy"><div class="battle-player-card rival"><div class="${enemyProfile.classes}" style="${enemyProfile.style}" data-aura-symbol="${safe(enemyProfile.aura)}"><span>${enemyProfile.avatar}</span></div><div><small>RIVAL CAPTAIN</small><b>${safe(enemy.name||"상대")}</b><em>${safe(enemy.title?`[${enemy.title}]`:"칭호 없음")}</em></div></div><div class="pvp-live-team">${pvpReplayTeamHtml(enemyTeam,enemySide)}</div></section><div id="pvpBattleAction" class="battle-action-card">한 마리씩 번갈아 공격합니다.</div></section></div>`);
  ...
  function maybeOpenLocalBattleReplayDemo(){
    ...
    const frames=[
      ...
      frame("아자토스의 맹목의 핵동\n치명타!\n파티 전체가 피해를 받았습니다.",1,62000000000,1,showSummons?demoSummons:[]),frame('<span class="battle-event battle-event--phase"><span class="battle-event__eyebrow">BOSS PHASE</span><b><span style="color:#d053ff">아자토스</span> · 최종 각성</b><span class="battle-event__body">전투 규칙이 바뀌고 두 번째 행동이 시작됩니다.</span></span>',2,34000000000,-1,showSummons?demoSummons:[]),frame("아자토스의 추가 공격\n대상 2 / 3\n2,000,000 피해",2,34000000000,0,showSummons?demoSummons:[]),frame('<span class="battle-event battle-event--ally"><span class="battle-event__eyebrow">ALLY SKILL</span><b>바다를 삼킨 태양 · 태양 폭발</b><span class="battle-event__body">태양 주기가 완성되어 전장을 밝힙니다.</span></span>',3,34000000000,-1,showSummons?demoSummons:[]),frame("[공허] 바다를 삼킨 태양 공격\n치명타!\n16,000,000,000 피해",3,18000000000,-1,showSummons?demoSummons:[]),
  ...
  function maybeOpenLocalPvpReplayDemo(){
    ...
    const voidEvent='<span class="battle-event battle-event--ally battle-event--void battle-event--void-observer"><span class="battle-event__eyebrow">VOID OBSERVATION</span><b><span style="color:#d96aff">[[PVP_SIDE:left]] 수상한 기운</span> · 뒤틀린 관측</b><span class="battle-event__body">상대의 공격을 분석해 효과를 뒤집었습니다.</span></span>';
    const frames=[snapshot("전투 시작",0),snapshot("턴 1",1),snapshot(voidEvent+"\n[[PVP_SIDE:left]] 수상한 기운의 반전 공격\n치명타!\n1,600,000 피해",1,[12000000,9800000],[7400000,10500000],"left","void-observer"),snapshot("PVP SIDE right 해신룡 공격\n900,000 피해",1,[11100000,9800000],[7400000,10500000],"right","rival-dragon"),snapshot("PVP SIDE left 수상한 기운 회피!",2,[11100000,9800000],[7400000,10500000],"right","rival-sun")];
    setTimeout(()=>openPvpBattleReplay({id:"local-pvp-ui-demo",left:{name:"로컬테스터",title:"관측자",profile:{}},right:{name:"도전자",title:"심해의 창",profile:{}},frames,winnerName:"로컬테스터",summary:"로컬 1대1 스킬 UI 검사 완료"}),80);
  }
  ...
  function applyLocalFusionDemo(){
    ...
    if(!params.has("fusionDemo")&&!params.has("presetDemo")&&!params.has("infoDemo")&&!params.has("recoveryDemo"))return;
    ...
    setTimeout(()=>params.has("infoDemo")?openMyInfo():params.has("recoveryDemo")?openRecoveryBattleConfirm([bucket[4]]):params.has("pvpPartyDemo")?openPvpPanel():params.has("presetDemo")?openPresets():openFusionLab(),80);
  }
  ...
  function openSaleConfirmation(displayNumber){
    ...
    openUiModal("판매 확인",`<div class="game-dialog confirm-sale-dialog"><div class="dialog-summary ${gradeClass(fish.grade)}"><div><small>SELL FISH</small><b>${safe(fish.name)}</b><p>${safe(fish.grade)} · 판매 금액 ${safe(formatMoney(finalPrice))}</p></div><span>${fishIcon(fish)}</span></div><div class="confirm-danger"><b>정말 판매할까요?</b><p>판매한 물고기는 되돌릴 수 없습니다.${isFishInPartyPreset(fish)?" 파티 프리셋에서도 제거됩니다.":""}</p></div><div class="dialog-actions"><button class="danger" data-confirm-sell="${Number(displayNumber)}">${safe(formatMoney(finalPrice))}에 판매</button><button onclick="closeModal()">취소</button></div></div>`);
  }

  function openBulkSaleConfirmation(){
    const sellable=bucket.filter(f=>!f.locked),total=sellable.reduce((sum,f)=>sum+applyMarketPrice(f),0),protectedCount=bucket.length-sellable.length;
    if(!sellable.length)return showToast(bucket.length?"판매 가능한 물고기가 없습니다.":"양동이가 비어 있습니다.");
    openUiModal("일괄판매 확인",`<div class="game-dialog confirm-sale-dialog"><div class="dialog-summary"><div><small>BULK SELL</small><b>${sellable.length.toLocaleString()}마리 일괄판매</b><p>잠금 물고기 ${protectedCount.toLocaleString()}마리는 안전하게 제외됩니다.</p></div><span>🪙</span></div><div class="confirm-danger"><b>정말 모두 판매할까요?</b><p>판매 대상에 포함된 프리셋 물고기도 함께 제거되며 되돌릴 수 없습니다.</p></div><div class="sale-total-preview"><small>예상 판매 금액</small><strong>${safe(formatMoney(total))}</strong></div><div class="dialog-actions"><button class="danger" data-confirm-bulk-sell>${safe(formatMoney(total))}에 일괄판매</button><button onclick="closeModal()">취소</button></div></div>`);
  }
  ...
  function bucketRenderKey() {
    return `${bucket.length}|${bucketSortOrder}|${fusionMainFishId}|${Object.values(fusionMainFishIds||{}).join(",")}|${bucket[0]?.id||""}|${bucket[bucket.length-1]?.id||""}`;
  }
  ...
  function openCosmetics(){
    const defaultCards=[
      ...
      cosmeticChoiceCard({type:"aura",id:"",name:"오라 없음",desc:"프로필 오라와 변형 프레임을 해제합니다.",icon:"○",unlocked:true,selected:!profileCosmetics.aura}),
    ...
    const auras=bossList.map(boss=>cosmeticChoiceCard({type:"aura",id:boss.id,name:boss.name+" 오라",desc:`${boss.name} 크레이지 최초 클리어 · 프레임 빛 단계 ${getProfileAuraTier(boss.id)}/5`,icon:bossSymbols[boss.id],color:boss.color,unlocked:isProfileCosmeticUnlocked("aura",boss.id),selected:profileCosmetics.aura===boss.id})).join("");
  ...
  function openRecoveryBattleConfirm(fishes=getPreparedRecoveryFishes()){
    ...
    openUiModal("회복 중인 물고기",`<div class="game-dialog recovery-confirm-dialog"><div class="dialog-summary warning"><div><small>PARTY CONDITION</small><b>부상 상태로 출전할까요?</b><p>현재 체력 그대로 레이드에 참가하며, 전투 후 다시 회복 시간이 적용될 수 있습니다.</p></div><span>❤️‍🩹</span></div><div class="recovery-confirm-list">${list}</div><div class="dialog-actions"><button class="primary" data-boss-confirm>현재 체력으로 시작</button><button onclick="closeModal()">파티 다시 선택</button></div></div>`);
  }
  globalThis.openRecoveryBattleConfirm=openRecoveryBattleConfirm;

  function openMyInfo(){
    if(!currentUser)return openUiModal("내정보",`<div class="game-dialog"><div class="dialog-summary"><div><small>CAPTAIN PROFILE</small><b>로그인이 필요합니다.</b></div><span>🪪</span></div></div>`);
    ...
    const maxRod=getCurrentPlayableMaxRod(),atRodCap=rodLevel>=maxRod,upgradeLabel=rodLevel>=MAX_ROD?"최대 레벨":atRodCap?"보스 처치 후 해금":`${getUpgradeSuccessRate(rodLevel).toFixed(1)}% · ${formatMoney(getUpgradeCost(rodLevel))}`;
    openUiModal("내정보",`<div class="game-dialog my-info-dialog"><section class="my-info-profile"><div class="${visual.classes}" style="${visual.style}" data-aura-symbol="${safe(visual.aura)}"><span>${visual.avatar}</span></div><div><small>MY CAPTAIN</small><h2>${safe(currentUser)}</h2><p>${safe(getCurrentTitle()?`[${getCurrentTitle()}] 장착 중`:"장착 칭호 없음")}</p></div><strong>${formatMoney(money)}</strong></section><div class="my-info-grid"><article><small>낚싯대</small><b>Lv.${Number(rodLevel||1).toLocaleString()} / ${MAX_ROD.toLocaleString()}</b></article><article><small>총 수익</small><b>${formatMoney(totalEarned)}</b></article><article><small>총 낚시</small><b>${Number(totalFishingCount||0).toLocaleString()}회</b></article><article><small>보유 물고기</small><b>${bucket.length.toLocaleString()}마리</b></article><article><small>도감</small><b>${collectionCount.toLocaleString()} / ${collectionTotal.toLocaleString()}</b></article><article><small>보스·업적</small><b>${bossClears.toLocaleString()}회 · ${achievementCount.toLocaleString()}개</b></article></div><section class="my-info-dashboard"><article class="my-info-panel"><header><div><small>PVP POWER</small><h3>최강 물고기 3마리</h3></div><strong>${compactNumber(combatPower)}</strong></header><div class="my-info-team">${teamHtml||'<div class="fusion-empty">전투 가능한 물고기가 없습니다.</div>'}</div></article><article class="my-info-panel"><header><div><small>ROD UPGRADE</small><h3>다음 강화</h3></div><strong>Lv.${Math.min(MAX_ROD,rodLevel+1).toLocaleString()}</strong></header><div class="my-info-upgrade"><i><em style="width:${Math.min(100,rodLevel/MAX_ROD*100)}%"></em></i><b>${upgradeLabel}</b><small>현재 해금 한도 Lv.${maxRod.toLocaleString()}</small></div></article></section><section class="my-info-growth"><h3>연구·훈련 현황</h3><div><span>🎣 낚시 연구 Lv.${Number(researchLevels?.fishing||0)} · 시간 -${getFishingTimeReduction()}%</span><span>💰 감정 연구 Lv.${Number(researchLevels?.appraisal||0)} · 판매 +${getAppraisalBonus()}%</span><span>⚔ 공격 훈련 Lv.${Number(trainingLevels?.attack||0)} · +${getTrainingAttackBonus()}%</span><span>❤️ 체력 훈련 Lv.${Number(trainingLevels?.hp||0)} · +${getTrainingHpBonus()}%</span><span>💥 치명타 피해 Lv.${Number(trainingLevels?.critDamage||0)} · +${getTrainingCritDamageBonus()}%</span><span>💨 일반~신화 도주 ${getEscapeChance().toFixed(1)}%</span></div></section><section class="my-info-panel my-info-chances"><header><div><small>FISHING RATE</small><h3>현재 등급 등장 확률</h3></div><strong>Lv.${rodLevel.toLocaleString()}</strong></header><div>${chanceHtml}</div></section><div class="dialog-actions my-info-actions"><button data-ui-action="cosmetics">프로필 꾸미기</button><button data-ui-action="titles">칭호 설정</button><button data-ui-action="settings">환경 설정</button></div></div>`);
  }
  ...
  function openBattleHistory(type="boss"){
    ...
    openUiModal("전투 기록",`<div class="game-dialog battle-history-dialog"><div class="dialog-summary"><div><small>RECENT BATTLE ARCHIVE</small><b>최근 전투를 다시 확인하세요</b><p>보스와 1대1 기록은 각각 최신 5회까지 저장됩니다.</p></div><span>📼</span></div>${tabs}<div class="history-list">${cards||`<div class="fusion-empty">아직 저장된 ${type==="boss"?"보스":"1대1"} 전투 기록이 없습니다.</div>`}</div></div>`);
  }
  ...
  function renderRankingView(type="menu"){
    ...
    const choices=[
      ...
      {id:"level",icon:"🎣",eyebrow:"ROD LEVEL",title:"낚싯대 레벨",detail:"낚싯대 성장 TOP 20"},
      {id:"pvp",icon:"⚔️",eyebrow:"PVP POWER",title:"PVP 전투력",detail:"출전 물고기 3마리 합산"}
    ];
    ...
    if(type==="menu")return openUiModal("랭킹",`<div class="game-dialog ranking-dialog"><div class="dialog-summary"><div><small>ONLINE RANKING</small><b>확인할 랭킹을 선택하세요</b><p>세 순위를 나눠서 필요한 목록만 불러옵니다.</p></div><span>🏆</span></div>${menu}</div>`);
  ...
  async function openRanking() {
    if(location.hostname==="127.0.0.1"&&new URLSearchParams(location.search).has("rankingDemo")){
      ...
      state.rankingSections={money:{icon:"💰",eyebrow:"WEALTH TOP 20",detail:"현재 보유 골드를 기준으로 정렬합니다.",rows:demoRows("누적 자산 순위","10조")},level:{icon:"🎣",eyebrow:"ROD LEVEL TOP 20",detail:"현재 낚싯대 레벨을 기준으로 정렬합니다.",rows:demoRows("낚싯대 성장 순위","Lv.2000")},pvp:{icon:"⚔️",eyebrow:"PVP POWER TOP 20",detail:"출전 물고기 3마리의 전투력을 합산합니다.",rows:demoRows("PVP 총합 전투력","9.8억")}};
    ...
    try {
      ...
      const pvpEntries=usersSnap.docs.map(doc=>{const u=doc.data(),game=u.gameState||{},fishes=Array.isArray(game.bucket)?game.bucket.filter(f=>f&&f.grade!=="쓰레기"&&f.combat):[],ids=Array.isArray(game.partyPresets?.pvp)?game.partyPresets.pvp.map(String):[];let team=ids.map(id=>fishes.find(f=>String(f.id||"")===id)).filter(Boolean).slice(0,3);if(team.length<3)team=[...fishes].sort((a,b)=>fishPower(b)-fishPower(a)).slice(0,3);return {u,team,power:team.reduce((sum,fish)=>sum+fishPower(fish),0)};}).filter(entry=>entry.team.length).sort((a,b)=>b.power-a.power).slice(0,20);
      ...
      state.rankingSections={money:{icon:"💰",eyebrow:"WEALTH TOP 20",detail:"현재 보유 골드를 기준으로 정렬합니다.",rows:section(moneySnap.docs,"money")},level:{icon:"🎣",eyebrow:"ROD LEVEL TOP 20",detail:"현재 낚싯대 레벨을 기준으로 정렬합니다.",rows:section(levelSnap.docs,"level")},pvp:{icon:"⚔️",eyebrow:"PVP POWER TOP 20",detail:"저장 프리셋 3마리, 프리셋이 없으면 전투력 상위 3마리를 합산합니다.",rows:pvpRows}};
  ...
  async function openPvpPanel() {
    ...
    if (!active) return openUiModal("1대1 대전", `<div class="game-dialog"><div class="dialog-summary"><div><small>ONLINE PVP</small><b>새로운 상대에게 도전하세요</b></div><span>⚔️</span></div><div class="dialog-form"><label for="pvpNickname">상대 닉네임</label><input id="pvpNickname" maxlength="16" placeholder="닉네임"><button data-pvp-request>대전 신청</button></div></div>`);
    ...
    if(active.status==="accepted"&&active.ready){
      return openUiModal("대전 준비 완료",`<div class="game-dialog pvp-ready-dialog"><div class="dialog-summary"><div><small>MATCH READY</small><b>VS ${safe(active.opponent||"상대")}</b><p>내 출전 파티 준비가 끝났습니다. 상대가 준비를 마치면 자동으로 전투가 시작됩니다.</p></div><span>✅</span></div><div class="pvp-ready-pulse"><i></i><b>상대 준비 대기 중</b><small>이 화면을 닫지 않아도 결과가 자동으로 표시됩니다.</small></div><div class="dialog-actions"><button data-pvp-cancel>대전 취소</button></div></div>`);
    }
  ...
  function openTransferHub(type="money"){
    ...
    if(!currentUser)return openUiModal("송금·물고기 전송",`<div class="game-dialog"><div class="dialog-summary"><div><small>SAFE TRANSFER</small><b>로그인이 필요합니다.</b><p>로그인 후 다른 선장에게 골드나 물고기를 보낼 수 있습니다.</p></div><span>🎁</span></div></div>`);
    ...
    if(type==="money")return openUiModal("송금·물고기 전송",`<div class="game-dialog transfer-dialog"><div class="dialog-summary"><div><small>SAFE MONEY TRANSFER</small><b>보유 골드 ${safe(formatMoney(money))}</b><p>서버에서 받는 사람과 잔액을 다시 확인한 뒤 한 번에 처리합니다.</p></div><span>💸</span></div>${tabs}<div class="dialog-form transfer-form"><label for="transferNickname">받는 사람 닉네임</label><input id="transferNickname" maxlength="16" autocomplete="off" placeholder="닉네임"><label for="transferAmount">송금할 금액</label><input id="transferAmount" inputmode="numeric" autocomplete="off" placeholder="1 이상의 정수"><p class="transfer-warning">거래가 완료되면 직접 되돌릴 수 없습니다. 닉네임과 금액을 확인해주세요.</p><button data-money-transfer>송금하기</button></div></div>`);
    const eligible=sortedBucketList().filter(entry=>!entry.fish.locked&&!isFusionMainFish(entry.fish)),eligibleIds=new Set(eligible.map(entry=>String(entry.fish.id)));state.transferFishIds=state.transferFishIds.filter(id=>eligibleIds.has(String(id)));
    ...
    return openUiModal("송금·물고기 전송",`<div class="game-dialog transfer-dialog"><div class="dialog-summary"><div><small>SAFE FISH TRANSFER</small><b>다중 선택 ${state.transferFishIds.length.toLocaleString()}마리</b><p>잠금 물고기와 합성 본체는 보호를 위해 목록에서 제외됩니다.</p></div><span>🐟</span></div>${tabs}<div class="dialog-form transfer-form"><label for="fishTransferNickname">받는 사람 닉네임</label><input id="fishTransferNickname" maxlength="16" autocomplete="off" placeholder="닉네임" value="${safe(state.transferTarget)}"><div class="transfer-fish-heading"><label>전송할 물고기</label><b>${state.transferFishIds.length.toLocaleString()}마리 선택</b></div><div class="transfer-fish-list">${cards||`<div class="fusion-empty">전송 가능한 물고기가 없습니다.</div>`}</div>${more}<p class="transfer-warning">선택한 물고기는 한 번의 안전한 거래로 전송되며 능력치·진화·합성·현재 체력을 유지합니다.</p><button data-fish-transfer ${state.transferFishIds.length?"":"disabled"}>선택한 ${state.transferFishIds.length.toLocaleString()}마리 전송하기</button></div></div>`);
  }
  ...
  function openInbox(){
    ...
    openUiModal("받은 소식",`<div class="game-dialog inbox-dialog"><div class="dialog-summary"><div><small>CAPTAIN INBOX</small><b>읽지 않은 소식 ${unread.toLocaleString()}개</b><p>메시지·송금·물고기 선물 기록은 직접 확인할 때까지 남아 있습니다.</p></div><span>📬</span></div><div class="inbox-actions"><button data-inbox-read-all ${unread?"":"disabled"}>모두 확인</button></div><div class="inbox-list">${cards||`<div class="fusion-empty">도착한 소식이 없습니다.</div>`}</div></div>`);
  }
  ...
  globalThis.showFishingLifeInboxNotice=count=>{renderInboxBadge();showGameNotice({icon:"📬",eyebrow:"CAPTAIN INBOX",title:"받은 소식 도착",detail:`읽지 않은 메시지·송금·선물 ${Number(count||0).toLocaleString()}개를 광장에서 확인할 수 있습니다.`,kind:"info",duration:4800});};
  async function sendDirectMessage(targetNickname, messageText) {
    ...
    } catch (error) { console.error(error); if(error.message==="USER_STATE_TOO_LARGE")showToast("상대 계정 데이터가 너무 커서 메시지를 보내지 못했습니다.");else if(await handleOnlineActionWriteError(error,currentUser,getGameState(),lastCloudSyncedState))showToast(error.message==="UPDATE_REQUIRED"?"최신 FishingLife 파일로 다시 접속해주세요.":"상대 계정 데이터가 안전 저장 형식과 맞지 않습니다.");else showToast(error.message === "TARGET_NOT_FOUND" ? "존재하지 않는 닉네임입니다." : "메시지를 보내지 못했습니다."); }
  ...
  document.addEventListener("click", async event => {
    ...
    if(versionRetryButton){
      ...
      if(status){status.className="login-gate-status loading";status.textContent="Firebase 서버에서 안전 버전을 확인하고 있습니다...";}
      const ok=await checkGameVersion(true);
      if(ok){
        if(status){status.className="login-gate-status success";status.textContent="확인 완료. 안전하게 다시 불러옵니다.";}
        if(currentUser)location.reload();
        else{closeModal(true);openLoginGate();}
      }else{
        versionRetryButton.disabled=false;
        if(status){status.className="login-gate-status error";status.textContent=cloudVersionGateMessage||"안전 버전을 확인하지 못했습니다.";}
      }
      return;
    }
    ...
    if(emergencyRestoreButton){
      ...
      if(result?.ok){
        ...
        showGameNotice({icon:"🛟",eyebrow:"ACCOUNT RESTORED",title:`${currentUser||"현재"} 계정 복구 완료`,detail:`Lv.${Number(result.summary?.rodLevel||1).toLocaleString()} · ${formatMoney(result.summary?.money||0)} · 물고기 ${Number(result.summary?.fishCount||0).toLocaleString()}마리`,kind:"success",duration:6500});
        return;
      }
      emergencyRestoreButton.classList.remove("loading");
      buttons.forEach(button=>button.disabled=false);
      if(result?.error==="SERVER_CHANGED"){openEmergencyAccountRecovery();showGameNotice({icon:"↻",title:"Firebase 데이터가 새로 바뀌었습니다",detail:"새로 도착한 송금·물고기를 잃지 않도록 최신값으로 다시 비교합니다.",kind:"info",duration:5000});return;}
      if(status){status.className="emergency-recovery-status error";status.textContent=result?.message||"복구 저장을 완료하지 못했습니다. 복구본은 삭제되지 않았습니다.";}
      return;
    }
