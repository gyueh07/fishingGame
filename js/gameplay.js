...
function applyPartyPreset(rawType){
  ...
  cleanPartyPresets();
  const fishes = getPresetFishList(type);
  if(fishes.length===0)return {ok:false,message:"저장된 "+(type==="boss"?"보스":"PVP")+" 프리셋이 없습니다."};
  ...
  if(available.length===0)return {ok:false,message:"프리셋 물고기가 모두 기절했거나 HP가 0입니다.",skipped};
...
function loadPartyPreset(rawType){
  ...
  if(!result.ok)return print(result.message||"프리셋을 불러오지 못했습니다.");
  print((result.type==="boss"?"보스":"PVP")+" 프리셋을 불러왔습니다.\n\n편성 : "+result.count+"마리"+(result.skipped?"\n기절·HP 0 제외 : "+result.skipped+"마리":""));
}
...
function finishOnlineTransferState(nextState,nextRevision,context={}){
  ...
  bossPrepIndexes=[];
  if(typeof shouldProtectEmergencyOriginalLocalSave!=="function"||!shouldProtectEmergencyOriginalLocalSave())localStorage.setItem("textFishingSpeciesSizeSave",JSON.stringify(getGameState()));
  updateWallet();
  setCloudSyncStatus(concurrentChanges?"saving":"saved",concurrentChanges?"전송 중 생긴 새 변경 내용을 이어서 Firebase에 저장합니다.":"전송 결과까지 Firebase에 저장되었습니다.");
  return true;
}
...
async function sendMoney(targetNickname, amount){
  ...
  try{
    ...
    await queueProtectedCloudAction(()=>db.runTransaction(async tx=>{
      ...
      const myWrite=txSetProtectedUser(tx,myRef,myData,{money:senderMoneyAfter,cloudRevision:myRevision,gameState:nextMyState},"money_transfer_send",{forceBackup:true});
      ...
      committed={state:myWrite.gameState,revision:myWrite.cloudRevision};
    }));
    ...
    if(e.message==="USER_STATE_TOO_LARGE")return transferFailure("계정 데이터가 너무 커서 안전하게 송금할 수 없습니다. 양동이 분리 저장 업데이트가 필요합니다.");
    if(await handleOnlineActionWriteError(e,currentUser,getGameState(),lastCloudSyncedState))return transferFailure(e.message==="UPDATE_REQUIRED"?"최신 FishingLife 파일로 다시 접속해주세요.":"상대 계정 데이터가 안전 저장 형식과 맞지 않아 송금을 중단했습니다.");
...
async function sendFish(targetNickname,displayNumber){
  ...
  try{
    ...
    await queueProtectedCloudAction(()=>db.runTransaction(async tx=>{
      ...
      const myRevision=Number(myData.cloudRevision||0)+1,targetRevision=Number(targetData.cloudRevision||0)+1;
      const myWrite=txSetProtectedUser(tx,myRef,myData,{cloudRevision:myRevision,gameState:nextMyState},"fish_transfer_send",{forceBackup:true});
      txSetProtectedUser(tx,targetRef,targetData,{cloudRevision:targetRevision,gameState:{...targetState,bucket:targetBucket,collection:targetCollection,notifications:targetNotifications,messages:targetMessages}},"fish_transfer_receive",{forceBackup:true});
      committed={state:myWrite.gameState,revision:myWrite.cloudRevision};
    }));
    ...
    if(e.message==="SYNC_REQUIRED")return transferFailure("최신 데이터를 저장하지 못했습니다. 네트워크를 확인한 뒤 다시 시도해주세요.");
    if(e.message==="PROGRESS_REGRESSION_BLOCKED")return transferFailure("진행도가 낮아지는 계정 저장을 차단했습니다. 관리자 확인이 필요합니다.");
    if(e.message==="USER_STATE_TOO_LARGE")return transferFailure("계정 데이터가 너무 커서 안전하게 전송할 수 없습니다. 양동이 분리 저장 업데이트가 필요합니다.");
    if(await handleOnlineActionWriteError(e,currentUser,getGameState(),lastCloudSyncedState))return transferFailure(e.message==="UPDATE_REQUIRED"?"최신 FishingLife 파일로 다시 접속해주세요.":"상대 계정 데이터가 안전 저장 형식과 맞지 않아 전송을 중단했습니다.");
...
async function sendFishBatch(targetNickname,fishIds){
  ...
  try{
    ...
    await queueProtectedCloudAction(()=>db.runTransaction(async tx=>{
      ...
      const myWrite=txSetProtectedUser(tx,myRef,myData,{cloudRevision:myRevision,gameState:nextMyState},"fish_batch_transfer_send",{forceBackup:true});
      txSetProtectedUser(tx,targetRef,targetData,{cloudRevision:targetRevision,gameState:{...targetState,bucket:targetBucket,collection:targetCollection,notifications:targetNotifications,messages:targetMessages}},"fish_batch_transfer_receive",{forceBackup:true});committed={state:myWrite.gameState,revision:myWrite.cloudRevision};
    }));
...
async function sendMessage(targetNickname){
  ...
  try{
    ...
    console.error(e);
    if(e.message==="USER_STATE_TOO_LARGE")return print("상대 계정 데이터가 너무 커서 메시지를 보내지 못했습니다.");
    if(await handleOnlineActionWriteError(e,currentUser,getGameState(),lastCloudSyncedState))return print(e.message==="UPDATE_REQUIRED"?"최신 FishingLife 파일로 다시 접속해주세요.":"상대 계정 데이터가 안전 저장 형식과 맞지 않아 메시지를 보내지 못했습니다.");
